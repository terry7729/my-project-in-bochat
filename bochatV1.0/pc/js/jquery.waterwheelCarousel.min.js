(function(a) {
	a.fn.waterwheelCarousel = function(r) {
		...
		function s() {
			...
			var x = c.separation;
			for(var w = 1; w <= c.flankingItems + 2; w += 1) {
				if(w > 1) {
		...
		function b() {
			...
			for(var u = 0; u < t.totalItems; u += 1) {
		...
		function q() {
			...
			for(var v = 1; v <= t.rightItemsCount; v += 1) {
				(u < t.totalItems - 1) ? u += 1: u = 0;
				t.items[u].css("opacity", 1);
				k(t.items[u], v)
			}
			var u = c.startingItem - 1;
			for(var v = -1; v >= t.leftItemsCount * -1; v -= 1) {
		...
		function p(v) {
			if(t.currentlyMoving === false) {
				...
				for(var y = 0; y < t.totalItems; y += 1) {
					var w = a(t.items[y]);
